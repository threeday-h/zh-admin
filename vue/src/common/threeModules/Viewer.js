...
export default class Viewer {
	...
	#initViewer() {
		...
		const animate = () => {
			...
			this.animateEventList.forEach(
				event => {
					event.fun && event.content && event.fun(event.content) // 执行动画
				}
			)
		}
	...
	#updateDom() {
		...
		this.renderer.setPixelRatio(window.devicePixelRatio) // 设置渲染器的像素比(是设备像素比)
	...
	#initRenderer() {
		...
		this.renderer.clearDepth() // 清空深度缓冲区
