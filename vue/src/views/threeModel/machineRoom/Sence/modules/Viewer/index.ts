  ...
  public initRaycaster() {
    ...
    const initRaycasterEvent: Function = (eventName: keyof HTMLElementEventMap): void => {
      const funWrap = throttle((event: any) => {
        ...
        // 修改：使用相对于容器的坐标，而不是相对于窗口的坐标
        this.mouse.x = ((event.clientX - rect.left) / this.viewerDom.clientWidth) * 2 - 1
        this.mouse.y = -((event.clientY - rect.top) / this.viewerDom.clientHeight) * 2 + 1

        // @ts-expect-error
        this.emitter.emit(Events[eventName].raycaster, this.getRaycasterIntersectObjects()) // 发射事件
      }, 50)
