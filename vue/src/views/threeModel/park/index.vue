<template>
  <div>
    ...
    <div class="container-main" ref="container" id="container"></div>
...
const init = () => {
  ...
  // skyBoxs = new SkyBoxs(viewer) // 天空盒子
  viewer.camera.position.set(17, 10, 52)
  viewer.controls.maxPolarAngle = Math.PI / 2.1 // 限制controls的上下角度范围
  // viewer.renderer.setPixelRatio(window.devicePixelRatio * 2) // 设置设备像素比

  viewer.renderer.shadowMap.enabled = true // 开启阴影
  viewer.renderer.shadowMap.type = THREE.PCFSoftShadowMap // 设置阴影模式
  ...
  const spotLights = new THREE.Group() // 聚光灯
  ...
  labelIns = new Labels(viewer) // 标签加载器

  viewer.addAxis() // 添加坐标轴
  viewer.addStats() // 添加状态检测
  // 初始化视频纹理
  initVideoTexture()
  // 初始化车辆
  loadCar()
...
const moveOnCurve = model => {
  if (curve && car) {
    if (progress <= 1 - velocity) {
      ...
      if (carObj.position.z.toFixed(2) >= 28.0 && carObj.position.z.toFixed(2) <= 28.1) {
        if (isopen.value) {
...
const loadOfficeBuild = () => {
  modelLoader.loadModelToScene("/glb/officeBuild.glb", model => {
    ...
    officeBuild = model
    officeBuild.openCastShadow() // 开启投射阴影
    officeBuild.openReceiveShadow() // 开启接收阴影
    //旋转360度
...
const onChangeTime = () => {
  ...
  const directionalLight = viewer.scene.getObjectByProperty("type", "DirectionalLight")
  ...
  if (timeText.value === TimeNums.night) {
    ...
    viewer.skyboxs.setSkybox("night")
    timeText.value = "白天模式"
